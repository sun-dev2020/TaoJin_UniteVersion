    ...
    if(self){
        ...
        if(!isShowBtn){
            if(isShowSeparatedLine){
                lineView = [[UIImageView alloc] initWithFrame:CGRectMake(Spacing2_0, 0.0f, kmainScreenWidth - Spacing2_0 * 2, 0.5f)];
            ...
            phoneLab = [self loadLabelWithFrame:CGRectMake(kmainScreenWidth - 56.0f - Spacing2_0, 0.0f, 56.0f, 0.0f) textAlignment:NSTextAlignmentCenter];
            ...
            self.beanLab = [self loadLabelWithFrame:CGRectMake(Spacing2_0, y + 3.0f + height, 50.0f, 11.0f) textAlignment:NSTextAlignmentRight];
            [self addSubview:self.beanLab];
            self.beanLab.hidden = isHideBeanLab;
            self.inforLab = [self loadLabelWithFrame:CGRectMake(self.beanLab.frame.origin.x + self.beanLab.frame.size.width + Spacing2_0 + 9.0f, y + 2.0f + height, kmainScreenWidth - self.beanLab.frame.origin.x - self.beanLab.frame.size.width - Spacing2_0 * 2, self.beanLab.frame.size.height) textAlignment:NSTextAlignmentLeft];
            [self addSubview:self.inforLab];
        }else{
            self.installBtn = [UIButton buttonWithType:UIButtonTypeCustom];
            self.installBtn.frame = CGRectMake(7.0f, 7.0f, kmainScreenWidth - Spacing2_0 * 2, 41.0f);
            [self.installBtn setBackgroundImage:[UIImage createImageWithColor:KGreenColor2_0] forState:UIControlStateNormal];
            [self.installBtn setBackgroundImage:[UIImage createImageWithColor:KLightGreenColor2_0] forState:UIControlStateHighlighted];
...
//    }
    ...
    if(beanNum > 0 && (state == 1 || state == 3)){
        ...
        _beanLab.textColor = KGrayColor2_0;
    }
    ...
    if(state == 3){
        if(![NSString isEmply:phoneTaskInfo]){
            //【审核中】
            _inforLab.frame = CGRectMake(_inforLab.frame.origin.x, _inforLab.frame.origin.y + height, kmainScreenWidth - self.beanLab.frame.origin.x - self.beanLab.frame.size.width - Spacing2_0 * 2 - phoneLab.frame.size.width, _inforLab.frame.size.height + 8.0f);
            [_inforLab sizeToFit];
            _inforLab.frame = CGRectMake(_inforLab.frame.origin.x, _inforLab.frame.origin.y, kmainScreenWidth - self.beanLab.frame.origin.x - self.beanLab.frame.size.width - Spacing2_0 * 2 - phoneLab.frame.size.width, _inforLab.frame.size.height + 8.0f);
            _inforLab.textColor = KOrangeColor2_0;
            phoneBackgroundView.hidden = NO;
            phoneLab.hidden = NO;
            phoneLab.text = @"审核中";
            phoneLab.backgroundColor = KGrayColor2_0;
        }
    }else if(state == 1){
        if(![NSString isEmply:phoneTaskInfo]){
            //【上传截图】
...
-(void)setTaskAppSubCellWithDescription:(NSString *)description{
    ...
    [_inforLab sizeToFit];
    _inforLab.frame = CGRectMake(_inforLab.frame.origin.x, 7.0f, kmainScreenWidth - 60.0f - Spacing2_0 * 2, _inforLab.frame.size.height);
}
